// economy.js
// โมดูลจัดการระบบเศรษฐกิจและไอเทม
// - ปรับให้เรียก RPC (Postgres functions) เพื่อทำธุรกรรมแบบ atomic
// - หาก RPC ยังไม่ถูกติดตั้ง จะ fallback ไปใช้ logic เดิม

window.useEconomy = function (sb, currentUser, loading, showToast, modals, CONFIG) {
    const { ref, computed } = Vue

    // ==========================================
    // 1. STATE (reactive refs) - exposed to UI
    // ==========================================
    const inventoryList = ref([])    // player's inventory rows (joined with items)
    const shopList = ref([])         // items available in shop

    // Bank state
    const bankAmount = ref(0)
    const bankMode = ref('deposit')  // 'deposit' | 'withdraw'

    // Transfer / form states
    const transferData = ref({ item: null, targetId: "", amount: 1 })
    const grantMoneyData = ref({ target: null, amount: 0 })     // admin grant money
    const transferMoneyData = ref({ targetId: "", amount: 0 })  // transfer between players
    const formItem = ref({})          // item create/edit form

    // ==========================================
    // 2. FETCH ACTIONS
    // ==========================================
    // Load shop items and current player's inventory
    async function fetchEconomyData() {
        if (!currentUser.value) return

        // Fetch shop items (sorted by buy price)
        const { data: shopData } = await sb.from('items').select('*').order('price_buy')
        shopList.value = shopData || []

        // Fetch inventory rows joined with items
        const { data: invData } = await sb
            .from('inventory')
            .select('*, items(*)')
            .eq('player_id', currentUser.value.id)
            .order('updated_at')
        inventoryList.value = invData || []
    }

    // ==========================================
    // 3. ITEM ACTIONS (Buy / Sell / Use / Equip / Discard)
    // ==========================================

    // Helper to detect function-missing error (simple)
    function rpcFunctionMissing(err) {
        if (!err || !err.message) return false
        const msg = err.message.toLowerCase()
        return msg.includes('does not exist') || msg.includes('function') && msg.includes('does not exist')
    }

    // Buy an item from the shop for currentUser
    // Now attempts to call RPC public.buy_item(...) to perform atomic purchase.
    // If RPC missing, falls back to legacy client-side sequence.
    async function buyItem(item) {
        if (!currentUser.value) return
        if ((currentUser.value.money || 0) < (item.price_buy || 0)) { showToast("เงินไม่พอ!", "error"); return }
        if (!confirm(`ซื้อ ${item.name} ราค��� ${item.price_buy}?`)) return

        loading.value = true
        try {
            // Try RPC first
            try {
                const params = {
                    player_id: currentUser.value.id,
                    item_id: item.id,
                    amount: 1
                }
                const { data, error } = await sb.rpc('buy_item', params)
                if (error) {
                    // If function doesn't exist, fallback to legacy flow below.
                    if (rpcFunctionMissing(error)) {
                        console.warn('RPC buy_item missing, falling back to legacy flow:', error.message)
                    } else {
                        showToast(error.message || 'ซื้อไม่สำเร็จ', 'error')
                        return
                    }
                } else {
                    // Supabase may return an array or object depending on function; normalize
                    const res = Array.isArray(data) ? data[0] : data
                    if (res && res.success) {
                        // Update UI, refresh inventory
                        if (res.new_money !== undefined) currentUser.value.money = res.new_money
                        showToast(`ซื้อ ${item.name} สำเร็จ`, "success")
                        await fetchEconomyData()
                        return
                    } else {
                        showToast((res && res.error) || 'ซื้อไม่สำเร็จ', 'error')
                        return
                    }
                }
            } catch (rpcErr) {
                // If any unexpected RPC error occurs, allow fallback path
                if (!rpcFunctionMissing(rpcErr)) {
                    // showToast(rpcErr.message || 'RPC error', 'error')
                    console.warn('RPC error (buy_item):', rpcErr)
                    // continue to fallback
                } else {
                    console.warn('RPC missing or not callable (buy_item):', rpcErr)
                }
            }

            // ---- Fallback legacy flow (non-atomic) ----
            // Deduct money from player
            const newMoney = currentUser.value.money - item.price_buy
            const { error: moneyErr } = await sb.from('players').update({ money: newMoney }).eq('id', currentUser.value.id)
            if (moneyErr) {
                showToast(moneyErr.message, 'error')
                return
            }

            // Update or insert inventory row
            const existing = inventoryList.value.find(i => i.item_id === item.id)
            if (existing) {
                await sb.from('inventory').update({ quantity: existing.quantity + 1 }).eq('id', existing.id)
            } else {
                await sb.from('inventory').insert([{ player_id: currentUser.value.id, item_id: item.id, quantity: 1 }])
            }

            // Update UI and notify
            currentUser.value.money = newMoney
            showToast(`ซื้อ ${item.name} สำเร็จ`, "success")
            await fetchEconomyData()
        } finally {
            loading.value = false
        }
    }

    // Sell an inventory item back to the shop (keeps legacy behavior)
    async function sellItem(invItem) {
        if (!confirm(`ขาย ${invItem.items.name} คืนร้านราคา ${invItem.items.price_sell}?`)) return
        loading.value = true
        try {
            const newMoney = (currentUser.value.money || 0) + (invItem.items.price_sell || 0)
            await sb.from('players').update({ money: newMoney }).eq('id', currentUser.value.id)

            if (invItem.quantity > 1) {
                await sb.from('inventory').update({ quantity: invItem.quantity - 1 }).eq('id', invItem.id)
            } else {
                await sb.from('inventory').delete().eq('id', invItem.id)
            }

            currentUser.value.money = newMoney
            showToast(`ขาย ${invItem.items.name} แล้ว`, "success")
            await fetchEconomyData()
        } finally {
            loading.value = false
        }
    }

    // Use a consumable / item that has effects
    async function useItem(invItem) {
        const effects = invItem.items.effects
        if (!effects || Object.keys(effects).length === 0) { showToast("ไอเทมนี้ไม่มีผลพิเศษ", "info"); return }

        loading.value = true
        try {
            const msg = []
            const updateData = {}

            // Heal HP
            if (effects.heal_hp) {
                const newHp = (currentUser.value.hp || 0) + effects.heal_hp
                updateData.hp = newHp
                msg.push(`ฟื้นฟู ${effects.heal_hp} HP`)
                currentUser.value.hp = newHp
            }

            // Buff stats (buff_{stat.key})
            CONFIG.stats.forEach(stat => {
                const buffKey = `buff_${stat.key}`
                if (effects[buffKey]) {
                    const val = effects[buffKey]
                    const currentVal = currentUser.value[stat.key] || 10
                    const newVal = currentVal + val

                    updateData[stat.key] = newVal
                    const newMod = Math.floor((newVal - 10) / 2)
                    updateData[stat.mod] = newMod

                    msg.push(`${stat.label} ${val > 0 ? '+' : ''}${val}`)
                    currentUser.value[stat.key] = newVal
                    currentUser.value[stat.mod] = newMod
                }
            })

            // Advance sequence (decrease sequence number)
            if (effects.advance_sequence) {
                let currentSeq = parseInt(currentUser.value.sequence) || 9
                if (currentSeq > 0) {
                    const newSeq = currentSeq - 1
                    updateData.sequence = newSeq.toString()
                    msg.push(`เลื่อนสู่ลำดับ ${newSeq}`)
                    currentUser.value.sequence = newSeq.toString()
                } else {
                    msg.push(`คุณอยู่ในลำดับสูงสุดแล้ว`)
                }
            }

            // Attack buff
            if (effects.buff_atk) {
                const newAtk = (currentUser.value.atk || 0) + effects.buff_atk
                updateData.atk = newAtk
                msg.push(`ATK ${effects.buff_atk > 0 ? '+' : ''}${effects.buff_atk}`)
                currentUser.value.atk = newAtk
            }

            // AC buff
            if (effects.buff_ac) {
                const newAc = (currentUser.value.ac || 10) + effects.buff_ac
                updateData.ac = newAc
                msg.push(`AC ${effects.buff_ac > 0 ? '+' : ''}${effects.buff_ac}`)
                currentUser.value.ac = newAc
            }

            // Persist updates and consume item if necessary
            if (Object.keys(updateData).length > 0) {
                const { error } = await sb.from('players').update(updateData).eq('id', currentUser.value.id)
                if (!error) {
                    if (invItem.items.type === 'consumable') {
                        if (invItem.quantity > 1) {
                            await sb.from('inventory').update({ quantity: invItem.quantity - 1 }).eq('id', invItem.id)
                        } else {
                            await sb.from('inventory').delete().eq('id', invItem.id)
                        }
                    }
                    showToast(`ใช้ไอเทม: ${msg.join(', ')}`, "success")
                    await fetchEconomyData()
                } else {
                    showToast(error.message, 'error')
                }
            } else {
                showToast("ใช้ไอเทมแล้วแต่ไม่มีอะไรเกิดขึ้น", "info")
            }
        } finally {
            loading.value = false
        }
    }

    // Discard an inventory item (no refund)
    async function discardItem(invItem) {
        modals.value.confirm = {
            title: 'ทิ้งไอเทม',
            message: `คุณต้องการทิ้ง ${invItem.items.name} ใช่หรือไม่? (ไม่ได้เงินคืน)`,
            type: 'delete',
            confirmText: 'ทิ้งเลย',
            onConfirm: async () => {
                modals.value.confirm = null
                loading.value = true
                try {
                    if (invItem.quantity > 1) {
                        await sb.from('inventory').update({ quantity: invItem.quantity - 1 }).eq('id', invItem.id)
                    } else {
                        await sb.from('inventory').delete().eq('id', invItem.id)
                    }
                    showToast("ทิ้งไอเทมแล้ว", "success")
                    await fetchEconomyData()
                } finally {
                    loading.value = false
                }
            }
        }
    }

    // Toggle equip / unequip for equipment items
    async function toggleEquip(invItem) {
        if (invItem.items.type !== 'equipment') return
        loading.value = true
        try {
            const newStatus = !invItem.is_equipped
            await sb.from('inventory').update({ is_equipped: newStatus }).eq('id', invItem.id)
            showToast(newStatus ? "สวมใส่แล้ว" : "ถอดออกแล้ว", "success")
            await fetchEconomyData()
        } finally {
            loading.value = false
        }
    }

    // ==========================================
    // 4. BANK & MONEY ACTIONS
    // ==========================================

    // Open bank modal (mode: 'deposit' | 'withdraw')
    function openBankModal(mode) {
        bankMode.value = mode
        bankAmount.value = 0
        modals.value.bank = true
    }

    // Execute deposit / withdraw (legacy flow remains)
    async function submitBankTransaction() {
        const amount = parseInt(bankAmount.value)
        if (amount <= 0) { showToast("กรุณาระบุจำนวนเงิน", "error"); return }

        loading.value = true
        try {
            let newMoney = currentUser.value.money
            let newBank = currentUser.value.bank_balance

            if (bankMode.value === 'deposit') {
                if (amount > newMoney) { showToast("เงินสดไม่พอฝาก", "error"); return }
                newMoney -= amount
                newBank += amount
            } else {
                if (amount > newBank) { showToast("ยอดเงินในธนาคารไม่พอ", "error"); return }
                newMoney += amount
                newBank -= amount
            }

            const { error } = await sb.from('players').update({ money: newMoney, bank_balance: newBank }).eq('id', currentUser.value.id)
            if (!error) {
                currentUser.value.money = newMoney
                currentUser.value.bank_balance = newBank
                showToast("ทำรายการสำเร็จ", "success")
                modals.value.bank = false
            } else {
                showToast(error.message, "error")
            }
        } finally {
            loading.value = false
        }
    }

    // Open transfer money modal
    function openTransferMoneyModal() {
        transferMoneyData.value = { targetId: "", amount: 0 }
        modals.value.transferMoney = true
    }

    // Submit transfer between players (uses bank_balance)
    // Use RPC transfer_bank for atomicity; fallback to legacy if RPC missing
    async function submitTransferMoney() {
        const { targetId, amount } = transferMoneyData.value
        if (!targetId || amount <= 0) { showToast("ข้อมูลไม่ถูกต้อง", "error"); return }
        if ((currentUser.value.bank_balance || 0) < amount) { showToast("ยอดเงินในธนาคารไม่พอ", "error"); return }

        loading.value = true
        try {
            // Try RPC
            try {
                const params = {
                    sender_player_id: currentUser.value.id,
                    receiver_player_id: targetId,
                    amount: amount
                }
                const { data, error } = await sb.rpc('transfer_bank', params)
                if (error) {
                    if (rpcFunctionMissing(error)) {
                        console.warn('RPC transfer_bank missing, falling back to legacy flow:', error.message)
                    } else {
                        showToast(error.message || 'โอนไม่สำเร็จ', 'error')
                        return
                    }
                } else {
                    const res = Array.isArray(data) ? data[0] : data
                    if (res && res.success) {
                        currentUser.value.bank_balance = res.sender_new_bank !== undefined ? res.sender_new_bank : currentUser.value.bank_balance - amount
                        showToast("โอนเงินสำเร็จ", "success")
                        modals.value.transferMoney = false
                        return
                    } else {
                        showToast((res && res.error) || 'โอนไม่สำเร็จ', 'error')
                        return
                    }
                }
            } catch (rpcErr) {
                console.warn('RPC error (transfer_bank):', rpcErr)
                // continue to fallback
            }

            // ---- Fallback legacy flow (non-atomic) ----
            // Deduct sender
            const newBalance = currentUser.value.bank_balance - amount
            await sb.from('players').update({ bank_balance: newBalance }).eq('id', currentUser.value.id)

            // Add to receiver (read current then update)
            const { data: target } = await sb.from('players').select('bank_balance').eq('id', targetId).single()
            if (target) {
                await sb.from('players').update({ bank_balance: (target.bank_balance || 0) + amount }).eq('id', targetId)
                currentUser.value.bank_balance = newBalance
                showToast("โอนเงินสำเร็จ", "success")
                modals.value.transferMoney = false
            } else {
                showToast("ไม่พบผู้รับ", "error")
            }
        } finally {
            loading.value = false
        }
    }

    // Admin: open grant money modal for a player
    function openGrantMoneyModal(player) {
        grantMoneyData.value = { target: player, amount: 0 }
        modals.value.grantMoney = true
    }

    // Admin: submit grant money (adds to bank_balance)
    async function submitGrantMoney() {
        const { target, amount } = grantMoneyData.value
        if (!target || amount <= 0) return
        loading.value = true
        try {
            const newBankBalance = (target.bank_balance || 0) + amount
            const { error } = await sb.from('players').update({ bank_balance: newBankBalance }).eq('id', target.id)
            if (!error) {
                showToast(`เสกเงิน ${amount} เข้าธนาคาร ${target.character_name} แล้ว`, "success")
                modals.value.grantMoney = false
                if (target.id === currentUser.value.id) currentUser.value.bank_balance = newBankBalance
            } else {
                showToast(error.message, "error")
            }
        } finally {
            loading.value = false
        }
    }

    // ==========================================
    // 5. ITEM TRANSFER & SHOP MANAGEMENT
    // ==========================================

    // Open transfer modal for an inventory item
    function openTransferModal(invItem) {
        transferData.value = { item: invItem, targetId: "", amount: 1 }
        modals.value.transfer = true
    }

    // Submit item transfer to another player
    async function submitTransfer() {
        const { item, targetId, amount } = transferData.value
        if (!targetId) { showToast("กรุณาเลือกผู้รับ", "error"); return }
        if (amount <= 0 || amount > item.quantity) { showToast("จำนวนไม่ถูกต้อง", "error"); return }

        loading.value = true
        try {
            // Remove or decrease from sender
            if (item.quantity === amount) {
                await sb.from('inventory').delete().eq('id', item.id)
            } else {
                await sb.from('inventory').update({ quantity: item.quantity - amount }).eq('id', item.id)
            }

            // Add to receiver (merge if exists)
            const { data: existing } = await sb.from('inventory').select('*').eq('player_id', targetId).eq('item_id', item.item_id).single()
            if (existing) {
                await sb.from('inventory').update({ quantity: existing.quantity + amount }).eq('id', existing.id)
            } else {
                await sb.from('inventory').insert({ player_id: targetId, item_id: item.item_id, quantity: amount })
            }

            showToast(`ส่งของเรียบร้อย`, "success")
            modals.value.transfer = false
            await fetchEconomyData()
        } finally {
            loading.value = false
        }
    }

    // Admin: open item CRUD modal (create/edit)
    function openItemModal(item = null) {
        if (item) {
            // prepare edit form: stringify effects for textarea
            formItem.value = { ...item, effects: JSON.stringify(item.effects, null, 2) }
        } else {
            // default new item form
            formItem.value = { name: '', description: '', type: 'consumable', price_buy: 0, price_sell: 0, image_url: '', effects: '{}' }
        }
        modals.value.item = true
    }

    // Submit item create / update
    async function submitItem() {
        loading.value = true
        try {
            const payload = { ...formItem.value }
            // parse effects JSON string to object
            payload.effects = JSON.parse(payload.effects)

            if (payload.id) {
                await sb.from('items').update(payload).eq('id', payload.id)
            } else {
                await sb.from('items').insert([payload])
            }

            showToast("บันทึกสินค้าสำเร็จ", "success")
            modals.value.item = false
            await fetchEconomyData()
        } catch (e) {
            showToast("รูปแบบ JSON ใน Effects ไม่ถูกต้อง", "error")
        } finally {
            loading.value = false
        }
    }

    // Delete an item from shop (admin)
    async function deleteItem(id) {
        if (!confirm("ยืนยันลบสินค้านี้?")) return
        loading.value = true
        try {
            await sb.from('items').delete().eq('id', id)
            showToast("ลบสินค้าแล้ว", "success")
            await fetchEconomyData()
        } finally {
            loading.value = false
        }
    }

    // ==========================================
    // 6. RETURN PUBLIC API (used by main app)
    // ==========================================
    return {
        // state
        inventoryList,
        shopList,
        bankAmount,
        bankMode,
        transferData,
        grantMoneyData,
        transferMoneyData,
        formItem,

        // actions
        fetchEconomyData,
        buyItem,
        sellItem,
        useItem,
        discardItem,
        toggleEquip,
        openBankModal,
        submitBankTransaction,
        openTransferModal,
        submitTransfer,

        // admin & money
        openGrantMoneyModal,
        submitGrantMoney,
        openTransferMoneyModal,
        submitTransferMoney,

        // shop management
        openItemModal,
        submitItem,
        deleteItem
    }
}